...
def _get_fw_configs() -> List[triton.Config]:  # noqa: C901
    ...
    # configs = [
    #     triton.Config(
    #         {"BLOCK_M": 64, "BLOCK_N": 32, "enable_tw_preload": True, "enable_pw_preload": True, "maxnreg": 167},
    #         num_stages=4,
    #         num_warps=4,
    #     ),
    # ]
    ...
    brute_force_maxnreg_search = False
    autotune_ops_and_maxnregs = os.getenv("HSTU_AUTOTUNE_OPS_AND_MAXNREGS", None) == "1"
    if autotune_ops_and_maxnregs:
        base_configs = configs
        configs = []
        for config in base_configs:
            if config.kwargs["BLOCK_M"] == 64 and config.kwargs["BLOCK_N"] == 32:
                for enable_preload in [False]:
                    for max_regs in range(96, 256) if brute_force_maxnreg_search else [128, 155, 256, 166, 207]:
                        configs.append(
                            triton.Config(
                                {
            ...
            configs.append(
                triton.Config(
                    {
                        "BLOCK_M": config.kwargs["BLOCK_M"], "BLOCK_N": config.kwargs["BLOCK_N"],
                        "enable_tw_preload": False, "enable_pw_preload": False,
                    },
                    num_stages=config.num_stages,
                    num_warps=config.num_warps,
                )
